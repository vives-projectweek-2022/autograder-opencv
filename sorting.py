...
# stap 3: Maak een nieuwe array, met subarrays per 5 om die daarna op X te kunnen sorteren
    def chunkPerFive(self, sortedY, chunks):
        #print("start")
        n=chunks
        chunkedPerFive=[sortedY[i:i + n] for i in range(0, len(sortedY), n)]
        #print("sorted into " + str(n) + " arrays")
...
# stap 4: sorteer op X
 ...
 # stap 6: X, Y waarden van de blobs vergelijken met die van de matrix uit stap 4
    def createAnswerMatrix(self):
        return None

    def sortAll(self):
        sortedY = self.sortY()
        chunks = self.chunkPerFive(sortedY, 5)
        print(chunks)
